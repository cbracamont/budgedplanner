...
var get_budget_summary_default = defineTool({
  ...
  handler: async (_input, ctx) => {
    ...
    const fixedIncome = sum(income.data, "amount");
    const monthVariableIncome = sum(variableIncome.data, "amount");
    ...
    const totalFixed = (fixed.data ?? []).filter(isDueThisMonth).reduce((acc, e) => acc + Number(e.amount ?? 0), 0);
